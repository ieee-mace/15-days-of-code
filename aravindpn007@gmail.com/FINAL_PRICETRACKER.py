...
def grab_price(product_code):
    ...
    try:
        ...
        try:
            ...
            # Some products in Amazon doesn't have price information or has a range (for eg: Rs 250 - 375)
            except:
                if product_code - 1 == 0:
                    message_label.destroy()
                    product_name_label.destroy()
...
root.title('Pricetracker for Amazon.in')
...
def on_focusout_2_product1(event):
    """ desired price here... reappears when we leave the field empty """
...
crall_frame = LabelFrame(root, padx=10, pady=7, borderwidth=2)
...
def reset_confirmation():
    """ Displays message box to confirm reset all """

    output = messagebox.askyesno('Reset all', 'Are you sure to reset all fields?')
    if output:
        reset_all()


reset_all_button = Button(crall_frame, text='Reset all', command=reset_confirmation, fg='red')
...
def analytics_product1(x):
    """ Displays plots of price fluctuation of a product """
    ...
    try:
        ...
        df1.plot(color=color_list[code - 1])
        plt.ylabel('Price')
        plt.show()

    except:
        var.set('Analytics')
        grab_price(1)  # Re-directs new user to enter user agent window.


# Drop down menu for Analytics
var = StringVar()
var.set('Analytics')
analytics_drop = OptionMenu(crall_frame, var, 'Product1', 'Product2', 'Product3', 'Product4', command=analytics_product1)
analytics_drop.grid(row=99, column=2)
